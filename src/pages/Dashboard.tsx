...
export const Dashboard: React.FC = () => {
  ...
  const [estimatedRevenue, setEstimatedRevenue] = useState('0.00');
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        const [metricsRes, movRes] = await Promise.all([
          api.get('/dashboard/metrics'),
          api.get('/stock-movements'),
        ]);

        setProductsCount(metricsRes.data.totalProducts);
        setLowStockCount(metricsRes.data.lowStockCount);
        setEstimatedRevenue(metricsRes.data.estimatedRevenue);
        setLossRate(metricsRes.data.lossRate);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-[#121215] border border-zinc-800/80 p-4 rounded-xl relative overflow-hidden shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-zinc-400 uppercase tracking-wider font-mono">Vendas Estimadas (7d)</span>
            <div className="w-8 h-8 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400">
              <DollarSign className="w-4 h-4" />
            </div>
          </div>
          <div className="mt-3">
            <h3 className="text-2xl font-bold text-white font-mono">R$ {estimatedRevenue}</h3>
            <div className="flex items-center gap-1 mt-1 text-[11px] text-emerald-400">
              <ArrowUpRight className="w-3.5 h-3.5" />
        ...
        <div className="bg-[#121215] border border-zinc-800/80 p-4 rounded-xl relative overflow-hidden shadow-sm">
          ...
          <div className="mt-3">
            <h3 className="text-2xl font-bold text-white font-mono">{lossRate}</h3>
            <div className="flex items-center gap-1 mt-1 text-[11px] text-emerald-400">
        ...
        <div className="lg:col-span-2 bg-[#121215] border border-zinc-800/80 p-5 rounded-xl">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-zinc-500">Volume de consumo registrado no balcão</p>
            </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {recentMovements.length === 0 ? (
                <p className="text-xs text-zinc-500 text-center py-8">Nenhuma movimentação registrada.</p>
              ) : (
                recentMovements.map((mov) => (
                  <div key={mov._id} className="flex items-center justify-between p-2.5 bg-[#18181B] border border-zinc-800/60 rounded-lg text-xs">
                    <div className="truncate pr-2">
                      <p className="font-semibold text-zinc-200 truncate">{mov.product?.name || 'Item'}</p>
